...
const Who = () => {
  ...
  return (
    <section className="bg-background w-full h-dvh md:h-screen flex flex-col xl:justify-center  xl:pt-15 ">
      <section className="flex h-2.5/7 md:h-4/7  xl:h-4/7   justify-between xl:gap-8 w-full align-middle items-center p-6 md:px-20 xl:px-40 relative ">
...
Dans la vie, j’aime beaucoup le dessin et l’animation. J’aime aussi cuisiner et les jeux vidéo."
        ...
        <img
          src="/mi.png"
          alt=""
          className=" hidden xl:block max-w-full object-cover xl:h-104.25 xl:w-75.56  xl:h-139 xl:w-100.75 z-2"
        />
        <img
          src="/line_14.png"
          alt=""
      ...
      <section className="flex flex-col xl:flex-row h-5/7 xl:h-3/7 bg-(--main-color)  gap-8 w-full items-center p-6 md:px-20 xl:px-40 justify-between selection:bg-(--sec-color) selection:text-(--main-color)!">
        <div className="flex flex-col gap-4 md:gap-2 xl:gap-4   justify-center items-center xl:hidden xl:flex">
          <H2
        ...
        <img
          ...
          className="max-w-full object-cover  h-40 w-73.32 xl:h-50 xl:h-60 xl:w-109.99"
        />
        <div className="flex flex-col gap-4   justify-center items-center  xl:w-2/7">
          <ul className="flex  justify-center flex-wrap xl:justify-start gap-4">
            {skills.map((skill) => (
              <li
                key={skill}
                className=" text-(--sec-color) px-2 py-1  md:px-4 md:py-2 border border-(--sec-color) rounded-full text-sm md:text-base xl:text-lg"
              >
                {skill}
              </li>
            ))}
